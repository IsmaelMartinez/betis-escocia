...
const showNavigation = (envConfig, description) => {
  ...
  const allNavigationItems = [
    ...
    { name: 'Clasificación', href: '/clasificacion', feature: 'NEXT_PUBLIC_FEATURE_CLASIFICACION' },
    { name: 'Coleccionables', href: '/coleccionables', feature: 'NEXT_PUBLIC_FEATURE_COLECCIONABLES' },
    { name: 'Galería', href: '/galeria', feature: 'NEXT_PUBLIC_FEATURE_GALERIA' },
    { name: 'Nosotros', href: '/nosotros', feature: 'NEXT_PUBLIC_FEATURE_NOSOTROS' },
    { name: 'Contacto', href: '/contacto', feature: 'NEXT_PUBLIC_FEATURE_CONTACTO' },
    { name: 'Historia', href: '/historia', feature: 'NEXT_PUBLIC_FEATURE_HISTORY' }
  ];
  
  const visibleItems = allNavigationItems.filter(item => 
    item.feature === null || process.env[item.feature] === 'true'
  );
  ...
  const hiddenItems = allNavigationItems.filter(item => 
    item.feature !== null && process.env[item.feature] !== 'true'
  );
...
console.log('   ✅ Flexibility: Easy to enable/disable features per environment');
