...
CREATE POLICY "Allow authenticated delete on rsvps" ON rsvps FOR DELETE TO authenticated USING (true);
-- Add policy to allow anonymous delete (for API routes that don't use auth)
-- This is needed for our current API structure
CREATE POLICY "Allow public delete on rsvps" ON rsvps FOR DELETE TO anon USING (true);
