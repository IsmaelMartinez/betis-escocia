...
- **External data**: football-data.org for matches and La Liga standings, cached server-side with `unstable_cache`
...
- API: `/api/matches`, `/api/standings` — both wrap `FootballDataService` with `unstable_cache`
...
export const GET = createApiHandler({
  schema: querySchema,
  handler: async (data) => {
    return { success: true, items: await fetchItems(data.type) };
  },
});
```
...
When a test or story needs match/standings data, use MSW handlers in `tests/msw/handlers.ts` rather than monkey-patching `fetch`.
