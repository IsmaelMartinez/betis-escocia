...
test.describe('Joaquín Page Happy Path', () => {
  ...
  test('should load successfully and display key elements', async ({ page }) => {
    ...
    await expect(page.locator('h1', { hasText: 'Los Chistes de Joaquín' })).toBeVisible();
    await expect(page.getByText('El hombre que convirtió el fútbol en comedia')).toBeVisible();
  ...
  test('should have external video links with correct attributes', async ({ page }) => {
    await page.goto('/joaquin');
  ...
  test('should render CTA section with navigation links', async ({ page }) => {
    ...
    const youtubeLink = page.getByRole('link', { name: /Más Vídeos en YouTube/ });
