...
test.describe.skip("Admin Dashboard", () => {
  ...
  test("should load admin dashboard successfully", async ({ page }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "Panel de Administración" }),
    ).toBeVisible();

    // Verify OneSignal notification panel is present
    await expect(
      page.getByTestId("onesignal-notification-panel"),
    ).toBeVisible();
    await expect(
      page.locator("h2", { hasText: "Notificaciones Push" }),
    ).toBeVisible();
  });

  test("should send test notification when toggle is enabled (simulated)", async ({
    page,
  }) => {
    ...
    if (await testButton.isVisible({ timeout: 2000 })) {
      ...
      await expect(
        page.locator('[data-testid="onesignal-notification-panel"]'),
      ).toBeVisible();
    } else {
      // If button not visible, at least verify the panel works
      console.log(
        "Test notification button not visible - panel may be in different state",
      );
    }
  ...
  test("should handle notification permission states correctly", async ({
    page,
  }) => {
    await page.goto("/admin");
    ...
    await expect(
      page.locator('label[for="notification-toggle"]'),
    ).toBeVisible();
    await expect(
      page.locator("text=Recibir notificaciones de actividad"),
    ).toBeVisible();
  });
  ...
  test("should show informational content about push notifications", async ({
    page,
  }) => {
    await page.goto("/admin");
    ...
    await expect(
      page.locator(
        "text=RSVP, mensajes de contacto y actividad administrativa",
      ),
    ).toBeVisible();
    await expect(
      page.locator(
        "text=Las notificaciones push te permiten recibir alertas en tiempo real",
      ),
    ).toBeVisible();
  });
});
