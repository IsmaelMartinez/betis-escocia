...
vi.mock('next/image', () => ({
  default: vi.fn(({ src, alt, width, height, className, unoptimized, ...props }) => (
    <img 
      src={src}
      alt={alt}
      width={width}
...
describe('ClassificationWidget Component', () => {
  ...
  describe('Error state', () => {
    it('renders error message when API fails', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Error al cargar la clasificación/)).toBeInTheDocument();
      });
  ...
  describe('Goal difference display', () => {
    it('shows negative goal difference with trending down', () => {
      ...
      render(<ClassificationWidget initialStandings={standingsWithNegativeGD} />);
    ...
    it('shows zero goal difference without trending icon', () => {
      ...
      expect(betisRow?.querySelector('[data-testid="trending-up-icon"]')).toBeNull();
      expect(betisRow?.querySelector('[data-testid="trending-down-icon"]')).toBeNull();
    });
  });
