...
describe('UpcomingMatchesWidget Component', () => {
  const mockMatches: (Match & { rsvp_count: number; total_attendees: number })[] = [
    {
      id: 1,
  ...
  describe('Empty state', () => {
    it('renders empty state when no matches', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Próximamente se anunciarán los siguientes partidos.')).toBeInTheDocument();
      });
    });
  ...
  describe('Success state with matches', () => {
    ...
    it('shows RSVP button for upcoming matches', async () => {
      ...
      await waitFor(() => {
        ...
        expect(rsvpButtons[0].closest('a')).toHaveAttribute('href', '/rsvp?match=1');
        expect(rsvpButtons[1].closest('a')).toHaveAttribute('href', '/rsvp?match=2');
      });
    });
    ...
    it('uses space-y layout for single match', async () => {
      vi.mocked(getUpcomingMatchesWithRSVPCounts).mockResolvedValueOnce([mockMatches[0]]);
    ...
    it('renders "Ver todos los partidos" link', async () => {
      ...
      await waitFor(() => {
        ...
        const bottomLink = links.find(link => link.textContent?.includes('partidos'));
  ...
  describe('Match display details', () => {
    it('handles matches without RSVP counts', async () => {
      ...
      vi.mocked(getUpcomingMatchesWithRSVPCounts).mockResolvedValueOnce(matchesWithoutRSVP);
    ...
    it('shows total attendees when available without confirmations', async () => {
      ...
      vi.mocked(getUpcomingMatchesWithRSVPCounts).mockResolvedValueOnce(matchesWithAttendeesOnly);
    ...
    it('handles past matches by not showing RSVP button', async () => {
      ...
      vi.mocked(getUpcomingMatchesWithRSVPCounts).mockResolvedValueOnce([pastMatch]);
