...
describe("StandardErrors", () => {
  describe("StandardErrors constants", () => {
    it("should have authentication and authorization errors", () => {
      ...
      expect(StandardErrors.ADMIN_REQUIRED).toBe(
        "Se requiere acceso de administrador",
      );
    });

    it("should have generic server errors", () => {
      expect(StandardErrors.INTERNAL_SERVER_ERROR).toBe(
        "Error interno del servidor",
      );
      expect(StandardErrors.SERVICE_UNAVAILABLE).toBe(
    ...
    it("should have data operation errors", () => {
      ...
      expect(StandardErrors.VALIDATION_ERROR).toBe(
    ...
    it("should have database errors", () => {
      ...
      expect(StandardErrors.DATABASE_CONNECTION_ERROR).toBe(
        "Error de conexión a la base de datos",
      );
    });

    it("should have voting specific errors", () => {
      expect(StandardErrors.VOTING.NOT_ACTIVE).toBe(
        "La votación no está activa en este momento",
      );
      expect(StandardErrors.VOTING.PERIOD_ENDED).toBe(
        "El período de votación ha terminado",
      );
      expect(StandardErrors.VOTING.ALREADY_VOTED).toBe(
        "Ya has votado anteriormente. Solo se permite un voto por persona",
      );
      expect(StandardErrors.VOTING.DESIGN_NOT_FOUND).toBe(
        "El diseño seleccionado no existe. Por favor, recarga la página",
      );
      expect(StandardErrors.VOTING.DATA_NOT_INITIALIZED).toBe(
        "No se encontraron datos de votación. La votación podría no estar inicializada",
      );
      expect(StandardErrors.VOTING.DATA_ERROR).toBe(
        "Error al cargar los datos de votación",
      );
      expect(StandardErrors.VOTING.INVALID_ACTION).toBe("Acción no válida");
      expect(StandardErrors.VOTING.PERMISSION_ERROR).toBe(
        "Error de permisos al acceder a los datos de votación",
      );
      expect(StandardErrors.VOTING.FORMAT_ERROR).toBe(
        "Error en el formato de los datos de votación",
      );
    });
  ...
  describe("getErrorMessage", () => {
    ...
    it("should use fallback with various error types", () => {
      ...
      expect(getErrorMessage([], fallbackMessage)).toBe(fallbackMessage);
      expect(
        getErrorMessage({ message: "not an error" }, fallbackMessage),
  ...
  describe("Integration with StandardErrors", () => {
    it("should work well with createStandardError for common patterns", () => {
      ...
      const validationError = createStandardError(
        StandardErrors.VALIDATION_ERROR,
        400,
      );
      ...
      expect(getErrorMessage(validationError)).toBe(
        StandardErrors.VALIDATION_ERROR,
      );
    ...
    it("should handle nested error objects", () => {
      const votingError = createStandardError(
