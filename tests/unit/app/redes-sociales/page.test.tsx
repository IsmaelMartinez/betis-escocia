...
vi.mock("@/components/FacebookPagePlugin", () => ({
  default: () => (
    <div data-testid="facebook-page-plugin">Facebook Page Plugin</div>
  ),
}));
...
vi.mock("lucide-react", () => ({
  ...
  Copy: ({ className, ...props }: any) => (
    <div data-testid="copy-icon" className={className} {...props}>
      Copy
    </div>
  ),
  ...
  Camera: ({ className, ...props }: any) => (
    <div data-testid="camera-icon" className={className} {...props}>
      Camera
    </div>
  ),
  Share2: ({ className, ...props }: any) => (
    <div data-testid="share2-icon" className={className} {...props}>
      Share2
    </div>
  ),
  Tag: ({ className, ...props }: any) => (
    <div data-testid="tag-icon" className={className} {...props}>
      Tag
    </div>
  ),
}));
...
describe("Redes Sociales Page", () => {
  ...
  describe("Basic Rendering", () => {
    ...
    it("should render quick tips section", () => {
      ...
      expect(
        screen.getByText("📸 Consejos Rápidos para Fotos Béticas"),
      ).toBeInTheDocument();
    ...
    it("should render Instagram section", () => {
      ...
      expect(
        screen.getByText("Plantillas listas para copiar y pegar"),
      ).toBeInTheDocument();
    ...
    it("should render Facebook section", () => {
      ...
      expect(
        screen.getByText("Posts más largos y descriptivos"),
      ).toBeInTheDocument();
  ...
  describe("Template Content", () => {
    it("should display correct Instagram template content", () => {
      ...
      expect(
        screen.getByText(/¡Qué ambiente en el Polwarth Tavern!/),
      ).toBeInTheDocument();
    ...
    it("should display correct Facebook template content", () => {
      ...
      expect(
        screen.getByText(/¡Nos vemos en el Polwarth Tavern para ver al Betis!/),
      ).toBeInTheDocument();
    });
  ...
  describe("Button Interactions", () => {
    it("should have hover effects on buttons", () => {
      ...
      const quickActionButtons = screen
        .getByText("Instagram Handle")
        .closest("button");
      expect(quickActionButtons).toHaveClass("hover:from-purple-600");

      const hashtagButton = screen
        .getByText("Essential Tags")
        .closest("button");
  ...
  describe("Content Sections", () => {
    it("should render tips content correctly", () => {
      ...
      expect(
        screen.getByText(
          "Capta el ambiente del día de partido: bufandas, banderas y sonrisas verdiblancas",
        ),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          "Incluye siempre #RealBetis #BetisEscocia #PeñaBéticaEscocesa para que todos nos encontremos",
        ),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          "Menciona @rbetisescocia en Instagram y etiqueta a la página de Facebook",
        ),
      ).toBeInTheDocument();
    });
