...
const StandingRowTest = ({ entry, isBetis }: { entry: StandingEntry; isBetis: boolean }) => {
  const positionBadge = getPositionBadge ? getPositionBadge(entry.position) : null;
  const formResults = formatForm ? formatForm(entry.form) : [];

  return (
    <tr className={`${isBetis ? 'bg-green-50 border-green-200' : 'hover:bg-gray-50'} transition-colors`} data-testid={`team-row-${entry.team.id}`}>
      {/* Position */}
      <td className="px-3 py-4 text-sm">
        <div className="flex items-center space-x-2">
          <span className={`font-medium ${getPositionStyle ? getPositionStyle(entry.position) : ''}`}>
            {entry.position}
          </span>
          {positionBadge && (
