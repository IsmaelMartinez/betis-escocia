  ...
  createApiHandler: vi.fn((config) => {
    return async (request: any) => {
      try {
        ...
        const schema =
          config.schema ??
          (config.i18nSchema ? config.i18nSchema((k: string) => k) : null);
        ...
        if (error && typeof error === "object" && "issues" in error) {
          ...
          errorResult.details = (error as any).issues.map(
        ...
        return {
          ...
          status:
            error && typeof error === "object" && "issues" in error ? 400 : 500,
        };
      }
    };
  }),
  ...
  const schema = {
    parse: vi.fn((data) => {
      if (!data.requestType) {
        ...
        error.issues = [
          { path: ["requestType"], message: "Request type is required" },
        ];
        throw error;
      }
      if (!["access", "deletion"].includes(data.requestType)) {
        const error = new Error("Validation failed") as any;
        error.issues = [
          { path: ["requestType"], message: "Invalid request type" },
        ];
        throw error;
      }
      return data;
...
describe("GDPR API", () => {
  ...
  describe("POST /api/gdpr", () => {
    ...
    it("should require authentication", async () => {
      ...
      vi.mocked(
        vi.mocked(await import("@clerk/nextjs/server")).getAuth,
      ).mockReturnValue({
        userId: null,
