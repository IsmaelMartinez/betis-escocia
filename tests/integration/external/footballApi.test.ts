...
describe("FootballDataService Integration Tests", () => {
  ...
  describe("fetchRealBetisMatches", () => {
      ...
      const mockMatches = [
        {
          ...
          homeTeam: { id: REAL_BETIS_TEAM_ID, name: "Real Betis" },
    ...
    it("should handle rate limiting errors (status 429)", async () => {
      ...
      const rateLimitErrorWithResponse = rateLimitError as Error & {
  ...
  describe("getBetisMatchesForSeasons", () => {
    it("should fetch matches for multiple seasons and combine them", async () => {
      ...
      const mockMatchesSeason2 = [
        {
          id: 2,
          utcDate: "2023-01-01T12:00:00Z",
          homeTeam: { id: REAL_BETIS_TEAM_ID, name: "Real Betis" },
          awayTeam: { id: 3, name: "Another" },
          status: "FINISHED",
        },
      ];
      ...
      mockAxiosInstance.get
        .mockResolvedValueOnce({
          data: { matches: mockMatchesSeason1 },
          status: 200,
        })
        .mockResolvedValueOnce({
          data: { matches: mockMatchesSeason2 },
          status: 200,
        });

      const seasons = ["2024", "2023"];
      const result =
        await footballDataService.getBetisMatchesForSeasons(seasons);
    ...
    it("should limit the number of returned matches", async () => {
      const mockMatches = Array.from({ length: 5 }, (_, i) => ({
        id: i + 1,
        utcDate: `2024-01-0${i + 1}T12:00:00Z`,
        homeTeam: { id: REAL_BETIS_TEAM_ID, name: "Real Betis" },
        awayTeam: { id: 2, name: "Opponent" },
        status: "FINISHED",
      }));
      ...
      const seasons = ["2024"];
      const limit = 3;
      const result = await footballDataService.getBetisMatchesForSeasons(
        seasons,
        limit,
      );
  ...
  describe("getUpcomingBetisMatchesForCards", () => {
    it("should return upcoming matches sorted by date", async () => {
      ...
      const mockMatches = [
        {
          id: 1,
          utcDate: futureDate1.toISOString(),
          homeTeam: { id: REAL_BETIS_TEAM_ID, name: "Real Betis" },
          awayTeam: { id: 2, name: "Opponent" },
          status: "SCHEDULED",
        },
        {
          id: 2,
          utcDate: futureDate2.toISOString(),
          homeTeam: { id: REAL_BETIS_TEAM_ID, name: "Real Betis" },
          awayTeam: { id: 3, name: "Another" },
          status: "SCHEDULED",
        },
      ...
      const result =
        await footballDataService.getUpcomingBetisMatchesForCards();
