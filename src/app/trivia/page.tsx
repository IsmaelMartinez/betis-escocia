...
export default function TriviaPage() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            if (
              response.status === 400 &&
              errorMessage.includes("jugado hoy")
            ) {
              setError(
                "Ya has jugado hoy. Tu puntuación anterior se mantiene. ¡Vuelve mañana para un nuevo desafío!",
              );
              return;
            }
        ...
        if (!error) {
          setError(
            "Error al guardar la puntuación. Tu juego fue completado pero la puntuación podría no haberse guardado.",
          );
        }
      }
    },
      ...
      async function initializeTriviaPage() {
        ...
        try {
          ...
          setError(
            error instanceof Error
              ? error.message
              : "An error occurred during initial check",
          );
          setGameState("error");
        }
      }
  ...
  if (gameState === "completed") {
    ...
    if (percentage === 100) {
      resultMessage =
        "¡Perfecto! ¡Conoces tu Betis y Escocia! ¡Vuelve mañana para otro desafío!";
      resultColor = "text-betis-verde";
    } else if (percentage >= 67) {
      resultMessage =
        "¡Muy bien! ¡Gran conocimiento de Betis y Escocia! ¡Inténtalo de nuevo mañana!";
      resultColor = "text-betis-verde";
    } else if (percentage >= 33) {
      resultMessage =
        "¡No está mal! ¡Estudia y vuelve mañana para otro intento!";
      resultColor = "text-yellow-600";
    } else {
      resultMessage =
        "¡Sigue aprendiendo sobre el Betis y Escocia! ¡Nuevas preguntas mañana!";
          ...
          {error && (
            <div className="mb-4 text-red-500 font-medium">
              ...
              {currentData.score !== null && (
                <p>Your score today: {currentData.score}</p>
              )}
            </div>
          )}
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-6">
        <p className="text-lg font-semibold mb-4">
          Pregunta {currentData.questionIndex + 1} of{" "}
          {currentData.questions.length}
        </p>
