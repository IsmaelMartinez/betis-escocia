...
async function syncMatches(user: { id: string; isAdmin: boolean } | null) {
...
// POST - Sync matches from external API (admin only)
export const POST = createApiHandler({
  auth: 'admin',
  handler: async (_, context) => {
    return await syncMatches(context.user || null);
  }
});
