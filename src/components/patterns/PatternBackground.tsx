...
export function TicketEdge({ 
  ...
  return (
    <div className={`pattern-ticket-edge ${className}`}>
      {children}
    </div>
  );
}
...
export function HeroBackground({ 
  ...
  return (
    <section className={`relative overflow-hidden ${variantClasses[variant]} ${className}`}>
      {/* Gold accent glow */}
...
export function CulturalCard({ 
  ...
  return (
    <div className={`pattern-card-cultural rounded-2xl border border-gray-200 shadow-lg ${className}`}>
      {children}
    </div>
  );
}
