...
export default function ShareMatch({ match, opponent }: ShareMatchProps) {
  ...
  const getShareText = (): string => {
    ...
    if (result) {
      ...
      const matchTime = format(new Date(match.utcDate), TIME_FORMAT, {
        locale: es,
      });
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      try {
        ...
        alert(
          "No se pudo compartir o copiar el enlace. Por favor, copia manualmente la URL de la página.",
        );
      }
    }
  };
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3">
      ...
      <button
        ...
        className="inline-flex items-center justify-center px-4 py-3 bg-betis-verde text-white font-medium rounded-lg hover:bg-betis-verde-dark transition-colors shadow-md"
