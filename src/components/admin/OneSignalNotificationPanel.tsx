...
const OneSignalNotificationPanel: React.FC<OneSignalNotificationPanelProps> = ({
  ...
  const loadPreference = useCallback(async () => {
    try {
      ...
      setState((prev) => ({
        ...prev,
        error:
          error instanceof Error ? error.message : "Error loading preferences",
        loading: false,
      }));
    }
  }, []);
  ...
  const initializeOneSignal = useCallback(async () => {
    ...
    console.log("[OneSignal] Current state:", {
      oneSignalReady: state.oneSignalReady,
    ...
    try {
      ...
      if (!process.env.NEXT_PUBLIC_ONESIGNAL_APP_ID) {
        throw new Error(
          "NEXT_PUBLIC_ONESIGNAL_APP_ID environment variable is not configured",
        );
      }
      ...
      const OneSignal = await import("react-onesignal").then(
        (mod) => mod.default,
      );
      console.log("[OneSignal] Import successful:", !!OneSignal);

      // Check if OneSignal is already initialized
      const windowWithOneSignal = window as unknown as {
      ...
      if (isAlreadyInitialized) {
        console.log(
          "[OneSignal] SDK already initialized, skipping init() call",
        );
      } else {
        ...
        if ("serviceWorker" in navigator) {
          ...
          console.log(
            "[OneSignal] Existing service workers:",
          ...
          for (const reg of registrations) {
            ...
            if (
              ...
              console.log(
                "[OneSignal] Keeping SW (likely OneSignal):",
                scriptURL,
              );
            }
          }
        ...
        console.log(
          "[OneSignal] Starting OneSignal initialization with config:",
          initConfig,
        );
        ...
        try {
          ...
          if (
            ...
            console.log(
              "[OneSignal] Already initialized (caught during init), continuing...",
            );
          } else {
      ...
      try {
        ...
        console.log(
          "[OneSignal] Using browser permission fallback:",
          browserPermission,
        );
      }
  ...
  const handleToggleChange = useCallback(
    async (enabled: boolean) => {
      try {
        ...
        console.log(
          "[OneSignal] Updating state with database result:",
          data.data,
        );
        setState((prev) => ({
          ...prev,
        ...
        if (enabled) {
          ...
          try {
              ...
              if (currentPermission === "default") {
                ...
                if (newPermission !== "granted") {
                  setState((prev) => ({
                    ...prev,
                    error:
                      "Se requieren permisos de notificación para activar las alertas.",
                  }));
                ...
                setState((prev) => ({
                  ...
                  error:
                    "Los permisos de notificación están denegados. Puedes habilitarlos en la configuración del navegador.",
                }));
                return;
              }
            }
            ...
            console.error(
              "[OneSignal] Browser permission request failed:",
              permError,
            );
            setState((prev) => ({
              ...prev,
          ...
          try {
            ...
            if (!initialized) {
              console.warn(
                "OneSignal initialization failed, but preference saved",
              );
              setState((prev) => ({
                ...prev,
                error:
                  "Preferencia guardada, pero hay problemas con OneSignal. Revisa la configuración.",
            ...
            setTimeout(async () => {
              try {
                ...
                console.log(
                  "[OneSignal] Current permission after init:",
                  currentPermission,
                );

                const isPermissionGranted = (perm: string | boolean) =>
                  perm === "granted" || perm === true;

                if (!isPermissionGranted(currentPermission)) {
                  console.log(
                    "[OneSignal] Requesting notification permissions...",
                  );
                  ...
                  try {
                    ...
                    console.log(
                      "[OneSignal] OneSignal permission request completed",
                    );
                  } catch (oneSignalError) {
                    console.warn(
                      "[OneSignal] OneSignal permission request failed, trying browser native:",
                      oneSignalError,
                    );
                    ...
                    if (
                      ...
                      console.log(
                        "[OneSignal] Browser permission request result:",
                        browserPermission,
                      );
                    }
                  }
                  ...
                  try {
                    ...
                    console.warn(
                      "[OneSignal] Could not get OneSignal permission, using browser permission:",
                      error,
                    );
                    newPermission = browserPermission;
                  }

                  console.log("[OneSignal] Final permissions:", {
                    oneSignal: newPermission,
                    browser: browserPermission,
                  });

                  // Use browser permission as the authoritative source
                  const finalPermissionGranted =
            ...
            console.log(
              "[OneSignal] OneSignal failed, requesting browser permissions directly...",
            );
            try {
              if (typeof Notification !== "undefined") {
                const browserPermission =
                  await Notification.requestPermission();
                console.log(
                  "[OneSignal] Direct browser permission result:",
                  browserPermission,
                );
          ...
          console.log(
            "[OneSignal] Disabling notifications, clearing states...",
          );
          setState((prev) => ({
            ...prev,
            permissionGranted: false,
  ...
  const handleTestNotification = useCallback(async () => {
    try {
      ...
      if (data.success) {
        console.log(
          "[OneSignal] OneSignal test notification sent successfully",
        );
        ...
        if (
          ...
          try {
            const browserNotification = new Notification(
              "🧪 Notificación de Prueba - Peña Bética",
              {
            ...
            console.warn(
              "[OneSignal] Could not show browser notification:",
              browserError,
            );
          }
        } else {
          console.warn(
            "[OneSignal] Browser notifications not available or not permitted",
          );
        }
      } else {
  ...
  useEffect(() => {
    if (state.enabled && !state.loading && !state.oneSignalReady) {
      console.log(
        "[OneSignal] Preferences loaded and enabled, initializing OneSignal...",
      );
      const initTimeout = setTimeout(async () => {
        try {
          ...
          if (success) {
            console.log(
              "[OneSignal] Initialization on preference load successful",
            );
          }
        } catch (error) {
          console.warn(
            "[OneSignal] Failed to initialize on preference load:",
            error,
          );
        }
      }, 500);
      ...
      console.log(
        "[OneSignal] OneSignal already ready, skipping preference-based initialization",
      );
    }
  ...
  return (
    <Card
      ...
      data-testid="onesignal-notification-panel"
    >
      <CardHeader>
